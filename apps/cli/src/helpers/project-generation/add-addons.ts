...
import { updateTviConfig } from "../../utils/tvi-config";
...
export async function addAddonsToProject(
	...
	try {
		...
		if (!isBetterTStack) {
			exitWithError(
				"This doesn't appear to be a tvi project. Please run this command from the root of a tvi project.",
			);
		}
		...
		if (!detectedConfig) {
			exitWithError(
				"Could not detect the project configuration. Please ensure this is a valid tvi project.",
			);
		}
